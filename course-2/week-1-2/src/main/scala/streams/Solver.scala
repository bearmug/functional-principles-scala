...
trait Solver extends GameDef {
  ...
  def str(init: List[Int]) : Stream[Int] = str(init) #::: str(init)

  /**
   * The stream of all paths that begin at the starting block.m
   */
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] =
    pathsFromStart
      .filter((tuple: (Block, List[Move])) => tuple match {
        case (b, _) if b.isStanding && goal == b.b1 => true
        case (_, _) => false
      })
